...
export const generate = (base: string = ".") => {
  ...
  if (fs.existsSync(path.join(base, "src", "config", "smtpd-app.yaml"))) {
    ...
    if (file) {
      ...
      for (let [type, value] of entries) {
        for (let id of value) {
          ...
          addHook(config, type, {
            ...
            data: fs
              .readFileSync(
                path.join(base, "src", "hooks", ...hookfolder, id + ".hsl"),
              )
              .toString(),
          });
        }
      ...
      if (fs.existsSync(filespath)) {
        ...
        const halonignores = files.filter(
          (file) => path.basename(file) === ".halonignore",
        );
        for (const halonignore of halonignores) {
          ...
          const patterns = buffer
            ...
            .filter((line) => line !== "");
          const folder = path
            .relative(filespath, path.dirname(halonignore))
            .split(path.sep)
            .join(path.posix.sep);
        ...
        for (let i of files) {
          ...
          const excluded = exclude.find((x) => {
            ...
            if (relative.startsWith("..")) {
              ...
              return minimatch(`${path.posix.sep}${relative}`, x[1], {
                matchBase: true,
              });
            }
          });
  ...
  if (
    fs.existsSync(path.join(base, "src", "config", "submission-tracking.yaml"))
  ) {
    const file = fs.readFileSync(
