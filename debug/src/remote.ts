    ...
    async (resolve, reject) => {
      ...
      if (config?.environment?.controlsocket !== undefined) {
        if (
          ...
          config.environment.controlsocket.path !== undefined
        ) {
          options = {
        ...
        if (
          "port" in config.environment.controlsocket &&
          config.environment.controlsocket.port !== undefined
        ) {
          options = {
...
export const hsh = (
  ...
  return new Promise<{ terminate: () => void; continue: () => void }>(
    async (resolve, reject) => {
      ...
      connector
        .openServerChannel(debugPath, (stream) => {
          var cmd = "e";
          channel.setupIPC(
            stream,
            (response) => {
